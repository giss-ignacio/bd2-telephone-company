...
CREATE TRIGGER TR_TICKET_ESTADO_RESUELTO 
  ON TICKET
  AFTER UPDATE
  AS
  BEGIN
  ...
  IF UPDATE (COD_ESTADO)
	...
	SELECT @nro_ticket=NRO_TICKET, @cod_estado=COD_ESTADO FROM INSERTED;
	IF (@cod_estado=4)
	BEGIN
		UPDATE TICKET
		SET FECHA_RESOLUCION=GETDATE() WHERE NRO_TICKET=@nro_ticket
	END
    END
  END
  ...
  -- VIEWS 
  CREATE VIEW dbo.v_tickets_resueltos
  WITH ENCRYPTION
  AS
	SELECT T.NRO_TICKET, C.NOMBRE, C.APELLIDO FROM TICKET T INNER JOIN CLIENTES C 
	ON T.NRO_DOC_CLI=C.NRO_DOC AND T.TIPO_DOC_CLI=C.TIPO_DOC
	WHERE COD_ESTADO=4
  GO

  CREATE VIEW dbo.v_servicios_inactivos
  WITH ENCRYPTION
  AS
	SELECT S.NRO_SERVICIO, TS.NOMBRE FROM SERVICIOS S 
	INNER JOIN TIPO_SERVICIO TS on S.ID_TIPO_SERVICIO=TS.ID_TIPO_SERVICIO
	WHERE S.COD_ESTADO_SERV=2
  GO

  CREATE VIEW dbo.v_clientes_con_mail_no_enviados
  WITH ENCRYPTION
  AS
	SELECT M.ID_MAIL, M.ASUNTO, C.NOMBRE, C.APELLIDO FROM MAIL M
	INNER JOIN CLIENTES C on M.DESTINATARIO_NRO_DOC=C.NRO_DOC AND M.DESTINATARIO_TIPO_DOC=C.TIPO_DOC
	WHERE M.FUE_ENVIADO=0
  GO
